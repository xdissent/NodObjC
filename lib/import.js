...
// "framework" is an alias to "import", since `import` is reserved JS
exports.framework = importFramework
...
var fs = require('fs')
  , ffi = require('ffi')
  , path = require('path')
  , debug = require('debug')('NodObjC:import')
  , core = require('./core')
...
function importFramework (framework, skip) {
  debug('importing framework (loading Classes? %s):', !skip, framework)
  ...
  var fw = {
    ...
    , lib: ffi.DynamicLibrary(frameworkPath)
  }
  ...
  if (!skip) {
    ...
    classes.forEach(function (c) {
      ...
      _global.__defineGetter__(c, function () {
        var clazz = core.objc_getClass(c)
        delete _global[c]
