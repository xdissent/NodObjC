...
function wrap (pointer, className) {
  debug('Class#wrap(%j, %s)', pointer.address(), className)
...
proto.addMethod = function addMethod (selector, type, func) {
  ...
  if (!this._methodCache) this._methodCache = {}
  this._methodCache[selector] = funcPtr
  return this
}
...
proto.toString = function toString () {
  return '[Class' +
    (this.isMetaClass() ? ' ◆' : '') + // the "meta key" symbol
    ': ' + this.getName() + ']'
}
