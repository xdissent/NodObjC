...
var Class = Object.create(ref.types.Utf8String)
Class.get = function get (buf, offset) {
  //console.error('Class getter!!!')
  if (offset !== 0) {
    buf = buf.slice(offset)
  }
  return require('./class').wrap(buf)
}
Class.set = function set (buf, offset, val) {
  //console.error('Class setter!!!')
  var classPtr = val ? val.pointer : ref.NULL
...
var objc = ffi.Library('libobjc', {
  ...
  // The SEL functions don't use a `SEL` type, because
  // the SEL type uses these functions internally
...
exports.dlopen = function dlopen (path) {
  return ffi.DynamicLibrary(path)
}
...
// FFI'd `free()` function
exports.free = ffi.ForeignFunction(exports.process.get('free'), 'void', [ voidPtr ])
