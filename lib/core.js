...
Class.get = function get (buf, offset) {
  ...
  if (offset !== 0) {
    debug('Class#get() slicing to offset', offset)
    buf = buf.slice(offset)
  }
  if (buf.readPointer().isNull()) {
    return null
  }
...
var objc = ffi.Library('libobjc', {
  ...
  // These SEL functions don't use a `SEL` type, because
...
// FFI'd `free()` function
var free = ffi.ForeignFunction(_process.get('free'), 'void', [ voidPtr ])
